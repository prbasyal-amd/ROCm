...
After installing ROCm, certain applications that are dependent on the Libva library (such as `vainfo`, `ffmpeg`) might fail to function correctly. This issue is only relevant if your are using Libva-based applications outside of ROCm on RHEL 8.10 and Oracle Linux 8. The failure occurs due to a symbol clash between the backported `libva-amdgpu` and the system-provided Libva. This conflict was introduced when adapting the RHEL 8 build to support additional operating systems, which required changes to build options. The issue will be fixed in a future ROCm release.
